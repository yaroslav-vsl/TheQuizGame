...
int MainFrame::GetCountQuestionAnswere()
{
    int count = 0;
    for (int index = 0; index < (int)questions.size(); index++)
    {
...
bool MainFrame::IsQuestion5Answered()
{
    int index = 4;
    if (index >= 0 && index < (int)questions.size()) {
        return questions[index].answered;
...
void MainFrame::CreateDefaultQuestions()
{
    ...
    std::vector<Question> defaultQuestions = {
        ...
        {"5. Сколько корней имеет уравнение? |x-3| = 5", {"0", "1", "2", "Бесконечно много"}, 1, false, "Уравнение |x - 3| = распадается на два линейных: 1) х - 3 = 5 → X = 8; 2) x - 3 = -5 → X = -2."},
        ...
        {"16. Сколько весит 1 кг ваты, если 1 кг железа утонул?", {"0,5 кг", "1 кг", "2 кг", "Зависит от объёма"}, 1, false, "Килограмм — единица массы, а не веса в бытовом смысле. 1 кг ваты = 1 кг железа по массе. Утонет или нет — зависит от плотности и выталкивающей силы, но на массу это не влияет"}
    };
