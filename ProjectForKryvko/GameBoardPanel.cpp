    ...
    : ImagePanel(parent, L""), mainFrame(mainFrame), isVideoPanelShown(false), videoPanel(nullptr), question_5_answered(false)
{
    ...
    size_button = wxSize(80, 80);
    size_button_video = wxSize(150, 60);
    radius_scale = 0.9;
    font_button = wxFont(12, wxFONTFAMILY_DEFAULT, wxFONTSTYLE_NORMAL, wxFONTWEIGHT_BOLD);

    if (size_frame.x > 2000)
    {
        size_button = wxSize(160, 160);
        size_button_video = wxSize(300, 120);
        radius_scale = 1.4;
        font_button = wxFont(22, wxFONTFAMILY_DEFAULT, wxFONTSTYLE_NORMAL, wxFONTWEIGHT_BOLD);
    }

    this->SetSize(size_frame);
    ...
    natureButton = new wxButton(this, wxID_ANY, "Природа", wxDefaultPosition, size_button_video);
    if (count < 16)
    {
    ...
    natureButton->SetFont(font_button);
    natureButton->Bind(wxEVT_BUTTON, &GameBoardPanel::OnNatureButtonClick, this);

    // Создаем кнопку "Жизнь" справа  
    lifeButton = new wxButton(this, wxID_ANY, "Жизнь", wxDefaultPosition, size_button_video);
    if (count < 16)
    {
        lifeButton->Disable();
    ...
    lifeButton->SetFont(font_button);
    ...
    for (int i = 0; i < 16; i++) 
    {
        ...
        {
            cellButtons[i] = new wxButton(buttonContainer, wxID_ANY, wxString::Format("%d", i + 1), wxDefaultPosition, size_button);
            cellButtons[i]->SetFont(font_button);
...
void GameBoardPanel::PositionButtonsInInfinityShape()
{
    ...
    int radius = std::min(centerX, centerY) * radius_scale;
    ...
    for (int i = 0; i < 16; i++) 
    {
        ...
        else
        {
            ...
            cellButtons[i]->SetPosition(wxPoint(posX - size_button.x / 2, posY - size_button.y / 2));
        }
    }
}
